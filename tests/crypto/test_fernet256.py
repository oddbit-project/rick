...
class TestFernet256:
    ...
    def test_key_validation(self):
        ...
        with pytest.raises(ValueError, match="Fernet key must be 64 url-safe base64-encoded bytes"):
    ...
    def test_decrypt_with_ttl(self, fernet):
        ...
        decrypted = fernet.decrypt_at_time(token, ttl=3600, current_time=current_time + 10)
        assert decrypted == data
        ...
        future_token = fernet.encrypt_at_time(data, current_time + 120)  # 120 seconds in future
        with pytest.raises(InvalidToken):
    ...
    def test_decrypt_at_time_requires_ttl(self, fernet):
        ...
        with pytest.raises(ValueError, match="decrypt_at_time\\(\\) can only be used with a non-None ttl"):
    ...
    def test_tampered_token_detection(self, fernet):
        ...
        tampered_tokens = [
            ...
            base64.urlsafe_b64encode(token_bytes[:-40] + b"\x00" * 8 + token_bytes[-32:]),
            # Tamper with HMAC
    ...
    def test_token_structure_validation(self, fernet):
        ...
        # Validate lengths
        assert len(decoded) >= 57  # Minimum token size: 1 + 8 + 16 + 16 + 32 = 73 bytes minimum
...
class TestMultiFernet256:
    ...
    def test_multifernet_creation(self, keys):
        ...
        with pytest.raises(ValueError, match="MultiFernet requires at least one Fernet instance"):
            MultiFernet256([])
    ...
    def test_multifernet_decrypt_with_ttl(self, multi_fernet):
        ...
        decrypted = multi_fernet.decrypt_at_time(token, ttl=3600, current_time=current_time + 10)
        assert decrypted == data
...
class TestFernet256Security:
    ...
    def test_randomness_quality(self):
        ...
        assert max(byte_counts) < len(iv_bytes) * 0.1  # No byte appears more than 10% of the time
...
class TestFernet256EdgeCases:
    ...
    def test_memory_efficiency(self):
        ...
        for size in [1024, 10240, 102400]:  # 1KB, 10KB, 100KB
            ...
            assert len(decoded_token) <= len(data) + expected_overhead + 16  # Max one block padding